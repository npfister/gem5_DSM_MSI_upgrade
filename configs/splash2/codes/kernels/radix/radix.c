...
struct prefix_node {
   ...
   int ranks[MAX_RADIX];
   parmacs_event_t (done);
   char pad[PAGE_SIZE];
};

struct global_memory {
   int Index;                             /* process ID */
   parmacs_lock_t (lock_Index);                    /* for fetch and add to get ID */
   parmacs_lock_t (rank_lock);                     /* for fetch and add to get ID */
   parmacs_lock_t (section_lock);  /* key locks */
   parmacs_barrier_t (barrier_rank);                   /* for ranking process */
   parmacs_barrier_t (barrier_key);                    /* for key sorting process */
   double *ranktime;
   double *sorttime;
   double *totaltime;
   int final;
...
int *key[2];            /* sort from one -1 into the other */
...

{
   ...
   global = (struct global_memory *) _parmacs_memory_alloc((int)(sizeof(struct global_memory)));
   key[0] = (int *) _parmacs_memory_alloc((int)(num_keys*sizeof(int)));;
   key[1] = (int *) _parmacs_memory_alloc((int)(num_keys*sizeof(int)));;
   key_partition = (int *) _parmacs_memory_alloc((int)((number_of_processors+1)*sizeof(int)));;
   rank_partition = (int *) _parmacs_memory_alloc((int)((number_of_processors+1)*sizeof(int)));;
   global->ranktime = (double *) _parmacs_memory_alloc((int)(number_of_processors*sizeof(double)));;
   global->sorttime = (double *) _parmacs_memory_alloc((int)(number_of_processors*sizeof(double)));;
   global->totaltime = (double *) _parmacs_memory_alloc((int)(number_of_processors*sizeof(double)));;
   size = number_of_processors*(radix*sizeof(int)+sizeof(int *));
   rank_me = (int **) _parmacs_memory_alloc((int)(size));;
   ...
   for (i=0;i<number_of_processors;i++) {
     gp[i].rank_ff = (int *) _parmacs_memory_alloc((int)(radix*sizeof(int)+PAGE_SIZE));;
   }
   {
    _parmacs_lockinit(&(global->lock_Index));
}
   {
    _parmacs_lockinit(&(global->rank_lock));
}
   {
    _parmacs_alockinit(&(global->section_lock), (MAX_PROCESSORS));
}
   {
    _parmacs_barinit(&(global->barrier_rank));
}
   {
    _parmacs_barinit(&(global->barrier_key));
}
   
   for (i=0; i<2*number_of_processors; i++) {
     {
    register int _parmacs_m4_count_ = 1;
    _parmacs_pauseinit(&(global->prefix_tree[i].done), _parmacs_m4_count_);
};
   }
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the key,
   ...
   for (toffset = 0; toffset < number_of_processors; toffset ++) {
     ...
     while ((offset & 0x1) != 0) {
       ...
       _index = base + offset;
       Place all addresses x such that:
         &(global->prefix_tree[_index]) <= x < 
              &(global->prefix_tree[_index + 1]) on node toffset
       base += level;
       level >>= 1;
     }  
   }  */
   ...
   {
    _parmacs_wait_for_end(number_of_processors - 1);
}
...
void slave_sort()
{
   ...
   {
    _parmacs_lock(global->lock_Index);
}
     MyNum = global->Index;
     global->Index++;
   {
    _parmacs_unlock(global->lock_Index);
}
...
/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
   ...
   {
    _parmacs_barrier((global->barrier_key), (number_of_processors));
} 
...
/* generate histograms based on one digit */
     ...
     {
    _parmacs_barrier((global->barrier_rank), (number_of_processors));
}  
     ...
     if ((MyNum & 0x1) == 0) {
        {
    register int _parmacs_m4_which_ = 0;
    _parmacs_setpause((global->prefix_tree[base + (offset >> 1)].done), _parmacs_m4_which_);
};
     }
     while ((offset & 0x1) != 0) {
       offset >>= 1;
       ...
       l = n - 1;
       _index = base + offset;
       n = &(global->prefix_tree[_index]);
       {
    register int _parmacs_m4_which_ = 0;
    _parmacs_waitpause((n->done), _parmacs_m4_which_);
};
       {
    register int _parmacs_m4_which_ = 0;
    _parmacs_clearpause((n->done), _parmacs_m4_which_);
};
       if (offset != (level - 1)) {
         for (i = 0; i < radix; i++) {
    ...
    _parmacs_barrier((global->barrier_rank), (number_of_processors));
     ...
     if (MyNum != (number_of_processors - 1)) {
       ...
       {
    register int _parmacs_m4_which_ = 0;
    _parmacs_waitpause((my_node->done), _parmacs_m4_which_);
};
       {
    register int _parmacs_m4_which_ = 0;
    _parmacs_clearpause((my_node->done), _parmacs_m4_which_);
};
       for (i = 0; i < radix; i++) {
