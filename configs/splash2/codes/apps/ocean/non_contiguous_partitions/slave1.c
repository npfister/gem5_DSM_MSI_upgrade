...

{
   ...
   {
    _parmacs_lock(locks->idlock);
}
     procid = global->id;
     global->id = global->id+1;
   {
    _parmacs_unlock(locks->idlock);
}
...
{
    _parmacs_barrier((bars->sl_prini), (nprocs));
}
   ...
   {
    _parmacs_barrier((bars->sl_psini), (nprocs));
}
...
/* compute input curl of wind stress */
   ...
   {
    _parmacs_barrier((bars->sl_onetime), (nprocs));
}
 ...
 ***************************************************************/
   ...
   while (!endflag) {
     while ((!dayflag) || (!dhourflag)) {
       ...
       if (nstep == 1) {
         if (procid == MASTER) {
            {(global->trackstart) = _parmacs_clock();}
         }
