...
CreateDistribution (cluster_type cluster, model_type model)
{
   ...
   particle_array = (particle *) _parmacs_memory_alloc((int)(Total_Particles * sizeof(particle)));;

   Particle_List = (particle **) _parmacs_memory_alloc((int)(Total_Particles * sizeof(particle *)));;
...
CreateParticleList (int my_id, int length)
{
   ...
   {
    _parmacs_lock(G_Memory->mal_lock);
};
   Local[my_id].Particles = (particle **) _parmacs_memory_alloc((int)(length 
						   * sizeof(particle *)));;
...
*/
   ...
   {
    _parmacs_unlock(G_Memory->mal_lock);
};
