...
/* set values of temparray{1,3} to psim{1,3}  */
   ...
   {
    _parmacs_barrier((bars->sl_phase_1), (nprocs));
}
...
/* to the values of the work1{1,2} arrays obtained from the
   ...
   {
    _parmacs_barrier((bars->sl_phase_2), (nprocs));
}
...
/* put the laplacian of the work7{1,2} arrays in the work4{1,2}
   ...
   {
    _parmacs_barrier((bars->sl_phase_3), (nprocs));
}
...
/* put the laplacian of the work4{1,2} arrays in the work7{1,2}
   ...
   {
    _parmacs_barrier((bars->sl_phase_4), (nprocs));
}
...
/*     *******************************************************
   ...
   {
    _parmacs_barrier((bars->sl_phase_5), (nprocs));
}
...
/*     *******************************************************
   ...
   if ((procid == MASTER) || (do_stats)) {
     {(multi_start) = _parmacs_clock();};
   }

   multig(procid);

   if ((procid == MASTER) || (do_stats)) {
     {(multi_end) = _parmacs_clock();};
...
/*  copy the solution for use as initial guess in next time-step  */
   ...
   {
    _parmacs_barrier((bars->sl_phase_6), (nprocs));
}
...
/* after computing its private sum, every process adds that to the
   ...
   {
    _parmacs_lock(locks->psibilock);
}
   global->psiai = global->psiai + psiaipriv;
   {
    _parmacs_unlock(locks->psibilock);
}

   {
    _parmacs_barrier((bars->sl_phase_7), (nprocs));
}
...
/*      *******************************************************
   ...
   {
    _parmacs_barrier((bars->sl_phase_8), (nprocs));
}
...
/*      *******************************************************
   ...
   {
    _parmacs_barrier((bars->sl_phase_9), (nprocs));
}
...
/* update the psi{1,3} matrices by adding 2*dtau*work3 to each */
   ...
   {
    _parmacs_barrier((bars->sl_phase_10), (nprocs));
}   
}
