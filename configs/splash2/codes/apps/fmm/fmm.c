...
/*  BDF (((Total_Particles/Number_Of_Processors) > 128) ? 2.0 : 3.0)*/
...
main (int argc, char *argv[])
{
   ...
   for (i = 1; i < Number_Of_Processors; i++) {
      {
    _parmacs_create(ParallelExecute);
};
   }
   ParallelExecute();
   {
...
ParallelExecute ()
{
   ...
   {
    _parmacs_barrier((G_Memory->synch), (Number_Of_Processors));
};
   {
    _parmacs_lock(G_Memory->count_lock);
};
     my_id = G_Memory->id;
     G_Memory->id++;
   {
    _parmacs_unlock(G_Memory->count_lock);
};   
...
/* POSSIBLE ENHANCEMENT:  Here is where one might reset the
   statistics that one is measuring about the parallel execution */
      ...
      if (MY_TIME_STEP == 2) {
        if (do_stats || my_id == 0) {
          {(local_init_done) = _parmacs_clock();};
        }
      } 
...
StepSimulation (int my_id, time_info *local_time, int time_all)
{
   ...
   if (time_all) 
      {(upward_end) = _parmacs_clock();};
   PartitionIterate(my_id, ComputeInteractions, BOTTOM);
   if (time_all) 
      {(interaction_end) = _parmacs_clock();};
   {
    _parmacs_barrier((G_Memory->synch), (Number_Of_Processors));
};
   if (time_all) 
      {(barrier_end) = _parmacs_clock();};
   PartitionIterate(my_id, DownwardPass, TOP);
   if (time_all) 
      {(downward_end) = _parmacs_clock();};
