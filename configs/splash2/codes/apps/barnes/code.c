#line 1 "../../../../parmacs.upc.3/pthreads/c.m4.pthreads"
...
string argv[];
{
   ...
   for(ProcessId = 1; ProcessId < NPROC; ProcessId++) {
      {
#line 140
    _parmacs_create(SlaveStart);
#line 140
};
   }

   /* Make the master do slave work so we don't waste the processor */
   {(Global->computestart) = _parmacs_clock();};
   printf("COMPUTESTART  = %12u\n",Global->computestart);
   SlaveStart();

   {(Global->computeend) = _parmacs_clock();};

   {
#line 150
    _parmacs_wait_for_end(NPROC-1);
#line 150
};
...
#line 175
   ...
   Global = (struct GlobalMemory *) _parmacs_memory_alloc((int)(sizeof(struct GlobalMemory)));;
   if (Global==NULL) error("No initialization for Global\n");
    
   {
#line 181
    _parmacs_barinit(&(Global->Barload));
#line 181
};
   {
#line 182
    _parmacs_barinit(&(Global->Bartree));
#line 182
};
   {
#line 183
    _parmacs_barinit(&(Global->Barcom));
#line 183
};
   {
#line 184
    _parmacs_barinit(&(Global->Baraccel));
#line 184
};
   {
#line 185
    _parmacs_barinit(&(Global->Barstart));
#line 185
};
   {
#line 186
    _parmacs_barinit(&(Global->Barpos));
#line 186
};

   {
#line 188
    _parmacs_lockinit(&(Global->CountLock));
#line 188
};
   {
#line 189
    _parmacs_lockinit(&(Global->io_lock));
#line 189
};
}
...
tab_init()
{
   ...
   for (i = 0; i < NPROC; ++i) {
      Local[i].ctab = (cellptr) _parmacs_memory_alloc((int)((maxcell / NPROC) * sizeof(cell)));;
      Local[i].ltab = (leafptr) _parmacs_memory_alloc((int)((maxleaf / NPROC) * sizeof(leaf)));;
   }

   /*allocate space for personal lists of body pointers */
   maxmybody = (nbody+maxleaf*MAX_BODIES_PER_LEAF)/NPROC; 
   Local[0].mybodytab = (bodyptr*) _parmacs_memory_alloc((int)(NPROC*maxmybody*sizeof(bodyptr)));;
   ...
   Local[0].mycelltab = (cellptr*) _parmacs_memory_alloc((int)(NPROC*maxmycell*sizeof(cellptr)));;
   Local[0].myleaftab = (leafptr*) _parmacs_memory_alloc((int)(NPROC*maxmyleaf*sizeof(leafptr)));;

   CellLock = (struct CellLockType *) _parmacs_memory_alloc((int)(sizeof(struct CellLockType)));;
   {
#line 262
    _parmacs_alockinit(&(CellLock->CL), (MAXLOCK));
#line 262
};
}
...
void SlaveStart()
{
   ...
   {
#line 273
    _parmacs_lock(Global->CountLock);
#line 273
};
     ProcessId = Global->current_id++;
   {
#line 275
    _parmacs_unlock(Global->CountLock);
#line 275
};
...
testdata()
{
   ...
   Local[0].tnow = 0.0;
   bodytab = (bodyptr) _parmacs_memory_alloc((int)(nbody * sizeof(body)));;
   if (bodytab == NULL) {
...
/* POSSIBLE ENHANCEMENT:  Here is where one might reset the
   statistics that one is measuring about the parallel execution */
    ...
    {
#line 529
    _parmacs_barrier((Global->Barstart), (NPROC));
#line 529
};

    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {(treebuildstart) = _parmacs_clock();};
    }
...
/*     B*RRIER(Global->Barcom,NPROC); */
    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {(partitionend) = _parmacs_clock();};
        Global->partitiontime += partitionend - partitionstart;
    }

    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {(forcecalcstart) = _parmacs_clock();};
    }

    ComputeForces(ProcessId);

    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {(forcecalcend) = _parmacs_clock();};
...
#line 602
    ...
    {
#line 607
    _parmacs_barrier((Global->Barpos), (NPROC));
#line 607
};

    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {(trackend) = _parmacs_clock();};
