...
char *argv[];
{
  ...
  gp = (struct gpid *) _parmacs_memory_alloc((int)(sizeof(struct gpid)));;
  gp->pid = 0;
  Global = (struct GlobalMemory *)
    _parmacs_memory_alloc((int)(sizeof(struct GlobalMemory)));;
  {
    _parmacs_barinit(&(Global->start));
}
  {
    _parmacs_lockinit(&(Global->waitLock));
}
  {
    _parmacs_lockinit(&(Global->memLock));
}
...
void Go()
{
  ...
  {
    _parmacs_lock(Global->waitLock);
}
    MyNum = gp->pid;
    gp->pid++;
  {
    _parmacs_unlock(Global->waitLock);
}

/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
   processors to avoid migration */

  lc =(struct LocalCopies *) _parmacs_memory_alloc((int)(sizeof(struct LocalCopies)+2*PAGE_SIZE));
  lc->freeUpdate = NULL;
  lc->freeTask = NULL;
  lc->runtime = 0;
