...

{
  ...
  last_malloc = (double **) _parmacs_memory_alloc((int)(P*sizeof(double *)));;
  for (i=0;i<P;i++) {
    proc_bytes[i] = 0;
    last_malloc[i] = NULL;
  }
  ...
  for (i=0;i<P;i++) {
    last_malloc[i] = (double *) _parmacs_memory_alloc((int)(proc_bytes[i] + PAGE_SIZE));
    if (last_malloc[i] == NULL) {
      fprintf(stderr,"Could not malloc memory blocks for proc %d\n",i);
      exit(-1);
    } 
    last_malloc[i] = (double *) (((unsigned long long) last_malloc[i]) + PAGE_SIZE -
                     ((unsigned long long) last_malloc[i]) % PAGE_SIZE);
...
/* Note that this causes all blocks to start out page-aligned, and that
  ...
  a = (double **) _parmacs_memory_alloc((int)(nblocks*nblocks*sizeof(double *)));;
  if (a == NULL) {
  ...
  rhs = (double *) _parmacs_memory_alloc((int)(n*sizeof(double)));;
  if (rhs == NULL) {
  ...
  Global->t_in_fac = (double *) _parmacs_memory_alloc((int)(P*sizeof(double)));;
  Global->t_in_mod = (double *) _parmacs_memory_alloc((int)(P*sizeof(double)));;
  Global->t_in_solve = (double *) _parmacs_memory_alloc((int)(P*sizeof(double)));;
  Global->t_in_bar = (double *) _parmacs_memory_alloc((int)(P*sizeof(double)));;
  Global->completion = (double *) _parmacs_memory_alloc((int)(P*sizeof(double)));;
...
*/
  ...
  {
    _parmacs_barinit(&(Global->start));
};
  {
    _parmacs_lockinit(&(Global->idlock));
};
  Global->id = 0;
  for (i=1; i<P; i++) {
    {
...

{
  ...
  {
    _parmacs_lock(Global->idlock);
}
    MyNum = Global->id;
    Global->id ++;
  {
    _parmacs_unlock(Global->idlock);
}
...

{
  ...
  {
    _parmacs_barrier((Global->start), (P));
};
