...
/*
 * CODE.H:  various global things for CODE.C.
 */
...
global struct CellLockType {
    parmacs_lock_t (CL);        /* locks on the cells*/
} *CellLock;
...
struct GlobalMemory  {	/* all this info is for the whole system */
    ...
    parmacs_barrier_t (Barstart);   /* barrier at the beginning of stepsystem  */
    parmacs_barrier_t (Bartree);    /* barrier after loading the tree          */
    parmacs_barrier_t (Barcom);     /* barrier after computing the c. of m.    */
    parmacs_barrier_t (Barload);    
    parmacs_barrier_t (Baraccel);   /* barrier after accel and before output   */
    parmacs_barrier_t (Barpos);     /* barrier after computing the new pos     */
    parmacs_lock_t (CountLock); /* Lock on the shared variables            */
    parmacs_lock_t (NcellLock); /* Lock on the counter of array of cells for loadtree */
    parmacs_lock_t (NleafLock);/* Lock on the counter of array of leaves for loadtree */
