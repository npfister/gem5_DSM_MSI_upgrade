...
UpwardPass (int my_id, box *b)
{
   ...
   if (b->type == CHILDLESS) {
      ...
      {
    _parmacs_alock((G_Memory->lock_array), (b->exp_lock_index));
};
      b->interaction_synch = 1;
      {
    _parmacs_aulock((G_Memory->lock_array), (b->exp_lock_index));
};
   }
   else {
   ...
   if (b->parent != NULL) {
      ...
      {
    _parmacs_alock((G_Memory->lock_array), (b->parent->exp_lock_index));
};
      b->parent->interaction_synch += 1;
      {
    _parmacs_aulock((G_Memory->lock_array), (b->parent->exp_lock_index));
};
   }
}
...
ShiftMPExp (int my_id, box *cb, box *pb)
{
   ...
   {
    _parmacs_alock((G_Memory->lock_array), (pb->exp_lock_index));
};
   for (i = 0; i < Expansion_Terms; i++) {
      COMPLEX_ADD((pb->mp_expansion[i]), (pb->mp_expansion[i]), result_exp[i]);
   }
   {
    _parmacs_aulock((G_Memory->lock_array), (pb->exp_lock_index));
};
}
...
XListInteraction (int my_id, box *source_box, box *dest_box)
{
   ...
   {
    _parmacs_alock((G_Memory->lock_array), (dest_box->exp_lock_index));
};
   for (i = 0; i < Expansion_Terms; i++) {
   ...
   {
    _parmacs_aulock((G_Memory->lock_array), (dest_box->exp_lock_index));
};
...
ShiftLocalExp (int my_id, box *pb, box *cb)
{
   ...
   {
    _parmacs_alock((G_Memory->lock_array), (cb->exp_lock_index));
};
   for (i = 0; i < Expansion_Terms; i++) {
   ...
   {
    _parmacs_aulock((G_Memory->lock_array), (cb->exp_lock_index));
};
}
