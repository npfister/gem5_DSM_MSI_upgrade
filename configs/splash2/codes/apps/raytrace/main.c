 ...
 *			_parmacs_memory_alloc((int)()); failed for some reason.
 *
...
VOID	StartRayTrace()
	...
	{
    _parmacs_lock(gm->pidlock);
}
	pid = gm->pid++;
	{
    _parmacs_unlock(gm->pidlock);
}

	if ((pid == 0) ||  (dostats))
        {(begin) = _parmacs_clock();};
	...
	{
    _parmacs_barrier((gm->start), (gm->nprocs));
}
...
CHAR	*argv[];
	...
	gm = (GMEM *)_parmacs_memory_alloc((int)(sizeof(GMEM)));;
	...
	{
    _parmacs_barinit(&(gm->start));
}
	{
    _parmacs_lockinit(&(gm->pidlock));
}
	{
    _parmacs_lockinit(&(gm->ridlock));
}
	{
    _parmacs_lockinit(&(gm->memlock));
}
	{
    _parmacs_alockinit(&(gm->wplock), (nprocs));
}
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the 
   ...
   desired.  */
	...
	for (i = 0; i < gm->nprocs - 1; i++)
		{
    _parmacs_create(StartRayTrace);
}

	StartRayTrace();

	{
    _parmacs_wait_for_end(gm->nprocs - 1);
}
	{(end) = _parmacs_clock();}
