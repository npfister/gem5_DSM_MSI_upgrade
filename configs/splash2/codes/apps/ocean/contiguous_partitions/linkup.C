...

{
  ...
  z = ((unsigned long long) q_multi + nprocs*sizeof(double ***));
  ...
  for (j=0;j<nprocs;j++) {
    zz = (unsigned long long) q_multi[j];
  ...
  z = ((unsigned long long) rhs_multi + nprocs*sizeof(double ***));
  if (nprocs%2 == 1) {         /* To make sure that the actual data
                                  starts double word aligned, add an extra
                                  pointer */
    z += sizeof(double ***);
  ...
  for (j=0;j<nprocs;j++) {
    zz = (unsigned long long) rhs_multi[j];
    zz += numlev*sizeof(double **);
    if (numlev%2 == 1) {       /* To make sure that the actual data
                                  starts double word aligned, add an extra
                                  pointer */
