...
MallocInit(P)
{
  ...
  mem_pool = (struct MemPool *)
    _parmacs_memory_alloc((int)((mallocP+1)*sizeof(struct MemPool)));;
  mem_pool++;
...
InitOneFreeList(p)
{
  ...
  {
    _parmacs_lockinit(&(mem_pool[p].memoryLock));
};
  if (p > 0) {
    mem_pool[p].freeBlock = (unsigned **)
      _parmacs_memory_alloc((int)((MAXFAST+1)*sizeof(unsigned *)));;
...
int size, home;
{
  ...
  if (result) {
    ...
    {
    _parmacs_lock(Global->memLock);
};
    freespace = (unsigned *) _parmacs_memory_alloc((int)(block_size+2*sizeof(unsigned)));;
    MigrateMem(freespace, block_size+2*sizeof(unsigned), home);

    mem_pool[home].touched++;
    {
    _parmacs_unlock(Global->memLock);
};
    freespace+=2;
