...
struct hLock {
	parmacs_lock_t (theLock);
	} *hashLock;

struct taskQ {
	parmacs_lock_t (taskLock);
...
InitTaskQueues(P)
{
  ...
  for (i=0; i<P; i++) {
    {
    _parmacs_lockinit(&(tasks[i].taskLock));
}
...
struct LocalCopies *lc;
{
  ...
  {
    _parmacs_lock(tasks[procnum].taskLock);
}
  ...
  {
    _parmacs_unlock(tasks[procnum].taskLock);
}
}
...
struct LocalCopies *lc;
{
  ...
  for (;;) {
    ...
    if (tasks[MyNum].taskQ || tasks[MyNum].probeQ) {
      {
    _parmacs_lock(tasks[MyNum].taskLock);
}
      t = NULL;
	...
	if (!t->next)
      ...
      {
    _parmacs_unlock(tasks[MyNum].taskLock);
}
      if (t)
        break;
    }
