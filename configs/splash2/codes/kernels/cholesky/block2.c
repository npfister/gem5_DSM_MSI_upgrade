...
int *T, *firstchild, *child, *PERM, *INVP, *domain, *partition;
{
  ...
  LB.entry = (Entry *) _parmacs_memory_alloc((int)(LB.entries_allocated*sizeof(Entry)));;
  MigrateMem(LB.entry, LB.entries_allocated*sizeof(Entry), DISTRIBUTED);
  LB.row = (int *) _parmacs_memory_alloc((int)(LB.entries_allocated*sizeof(int)));;
...
FindMachineDimensions(P)
{
  ...
  for (_try=(int) sqrt((double) P); _try>0; _try--) {
    div = P/_try;
    if (div*_try == P)
      break;
  }
