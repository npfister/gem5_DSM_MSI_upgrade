  ...
  unsigned ProcID;
{
    ...
    {
    _parmacs_lock(gl->InterfVirLock);
};
    *VIR = *VIR + LVIR;
    {
    _parmacs_unlock(gl->InterfVirLock);
};
    ...
    if (comp_last > NMOL1) {
        for (mol = StartMol[ProcID]; mol < NMOL; mol++) {
            {
    _parmacs_alock((gl->MolLock), (mol % MAXLCKS));
}; 
            ...
            {
    _parmacs_aulock((gl->MolLock), (mol % MAXLCKS));
};
        }
        comp = comp_last % NMOL;
    ...
    {
    _parmacs_barrier((gl->InterfBar), (NumProcs));
};
