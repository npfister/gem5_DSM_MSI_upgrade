...
typedef struct	gmem
	...
	parmacs_barrier_t (start);			/* Barrier for startup sync.	     */
	parmacs_lock_t (pidlock);		/* Lock to increment pid.	     */
	parmacs_lock_t (ridlock);		/* Lock to increment rid.	     */
	parmacs_lock_t (memlock);		/* Lock for memory manager.	     */
	parmacs_lock_t (wplock);	/* Locks for shared work pools.      */
    UINT par_start_time;
...
extern	void send_ray(struct ray *r,struct voxel *v);
