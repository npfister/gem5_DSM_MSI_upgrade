...
InsertBoxInGrid (int my_id, box *b, box *pb)
{
   ...
   if (pb == NULL) {
      {
    _parmacs_lock(G_Memory->single_lock);
};
      if (Grid == NULL) {
	 Grid = b;
	 success = TRUE;
	 ...
	 success = FALSE;
      {
    _parmacs_unlock(G_Memory->single_lock);
};
   }
   else {
      {
    _parmacs_alock((G_Memory->lock_array), (pb->particle_lock_index));
};
	 ...
	 success = FALSE;
      {
    _parmacs_aulock((G_Memory->lock_array), (pb->particle_lock_index));
};
   }
   if (success == TRUE)
