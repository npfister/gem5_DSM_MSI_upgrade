  ...
  
{
    ...
    _parmacs_main_initenv(_parmacs_m4_memory_);
        ...
        start_end = (first_last_array **) 
            _parmacs_memory_alloc((int)(sizeof(first_last_array *) * NumProcs));;
        for (i=0; i < NumProcs; i++) {
            start_end[i] = (first_last_array *)
                _parmacs_memory_alloc((int)(sizeof(first_last_array)));;
        }
        ...
        my_boxes = (box_list **) _parmacs_memory_alloc((int)(NumProcs * sizeof(box_list *)));;
        ...
        BOX = (box_type ***) _parmacs_memory_alloc((int)(BOX_PER_SIDE * sizeof(box_type **)));;
        for (i=0; i < BOX_PER_SIDE; i++) {
            BOX[i] = (box_type **) _parmacs_memory_alloc((int)(BOX_PER_SIDE * sizeof(box_type *)));;
            for (j=0; j < BOX_PER_SIDE; j++) {
                BOX[i][j] = (box_type *) _parmacs_memory_alloc((int)(BOX_PER_SIDE * sizeof(box_type)));;
                for (k=0; k < BOX_PER_SIDE; k++) {
                    BOX[i][j][k].list = NULL;
                    {
    _parmacs_lockinit(&(BOX[i][j][k].boxlock));
};
                }
            }
        } /* for i */
