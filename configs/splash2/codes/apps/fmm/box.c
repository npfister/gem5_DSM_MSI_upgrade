...
CreateBoxes (int my_id, int num_boxes) 
{
   ...
   Local[my_id].B_Heap = (box *) _parmacs_memory_alloc((int)(num_boxes * sizeof(box)));;
 ...
 */
void
PrintBox (int dummy, box *b)
{
   int i;
  
   {
    _parmacs_lock(G_Memory->io_lock);
};
   fflush(stdout);
   if (b != NULL) {
      printf("Info for B%f :\n", b->id);
   ...
   {
    _parmacs_unlock(G_Memory->io_lock);
};
}
